...
function BannerAd() {
  return (
    ...
    >
      ...
      <span style={{ fontSize: 12, color: '#86868b' }}>広告枠(AdMob / Ad Manager 差し込み予定)</span>
    </div>
  );
}
...
function TravelPromo({ fixture, team }) {
  ...
  // 対戦相手の最寄り駅名でエリアを絞り込んだ楽天トラベルのキーワード検索結果へ
  // 誘導し、それをアフィリエイトリンク経由でラップする。
  const areaKeyword = encodeURIComponent(`${fixture.host.travelOrigin ? fixture.host.travelOrigin.label : fixture.host.stadium} ホテル`);
  const areaSearchUrl = `http://kw.travel.rakuten.co.jp/keyword/Search.do?f_query=${areaKeyword}`;
  const rakutenAffiliateUrl = `https://hb.afl.rakuten.co.jp/hgc/55b1ae55.b3829984.55b1ae56.07fa8f71/_RTLink137897?pc=${encodeURIComponent(areaSearchUrl)}&link_type=text&ut=eyJwYWdlIjoidXJsIiwidHlwZSI6InRleHQiLCJjb2wiOjF9`;

  const links = [
    {
      label: `${fixture.host.travelOrigin ? fixture.host.travelOrigin.label : fixture.host.short}周辺のホテルを探す`,
      // 楽天アフィリエイトの実リンク。行き先の駅名でキーワード検索結果に絞り込んでいる
      url: rakutenAffiliateUrl,
    },
    {
      label: '新幹線・列車の時刻を調べる',
...
export default function JLeagueTicketApp() {
  ...
  const pastMatchCount = fixtures.length - fixtures.filter((f) => f.matchDate >= TODAY_DATE_ONLY).length;
      ...
      <div style={{ maxWidth: 480, margin: '0 auto' }}>
        ...
        <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 16 }}>
          {visibleFixtures.length === 0 && (
            <div style={{ textAlign: 'center', color: '#6e6e73', fontSize: 13, padding: '32px 0' }}>
              該当する試合がありません。
            </div>
          )}
